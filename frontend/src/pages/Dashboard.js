...
function StatCard({ label, value, tone = "default" }) {
  const toneClass =
    tone === "accent" ? "surface-tint text-app" : "surface-card text-app";
...
function PlanLockNotice({ title, message }) {
  return (
    <div className="rounded-3xl border border-amber-300/35 bg-amber-300/10 p-6 text-amber-700">
      ...
      <p className="mt-4 text-xs uppercase tracking-[0.2em] text-amber-700 dark:text-amber-200">
...
function CompanyUsersPanel({ companyName, users, loading, error, onRetry }) {
  if (loading) {
    return (
      <div className="surface-card p-6 text-sm text-app-secondary">
    ...
    return (
      <div className="surface-card border-dashed border-cyan-300/30 p-10 text-center">
        ...
        <p className="text-app-secondary mt-2 text-sm">
          No user records are currently available for {companyName}.
        </p>
      </div>
    );
  }
    ...
    <div className="surface-card overflow-hidden">
      ...
      {users.map((user) => {
        ...
        return (
          <div
            key={user.user_id}
            className="grid grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)_120px_100px_170px] gap-4 border-t border-app px-5 py-4 text-sm text-app-secondary"
          >
...
function AdminAnalyticsPanel({ data, loading, error, onRetry, planFeatures }) {
  const analyticsEnabled =
    data?.analytics_enabled || Boolean(planFeatures?.admin_analytics);
  if (!analyticsEnabled) {
    return (
      <PlanLockNotice
        title="Company Analytics"
        message="Company analytics requires a higher plan. Upgrade to unlock scan trends and risk distribution."
      />
    );
  }
  if (loading) {
  ...
  const fileTotal = Object.values(fileTypes).reduce(
    (acc, count) => acc + count,
    0,
  );
    ...
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          value={summary.scans_this_week ?? 0}
        />
        <StatCard
          label="High Risk Scans"
          value={summary.high_risk_scans ?? 0}
        />
        <StatCard
          label="PDF Reports"
          value={summary.report_activity?.pdf_reports_downloaded ?? 0}
        />
      </div>

      <div className="grid gap-4 lg:grid-cols-3">
        <section className="surface-card p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-app-muted">
            Risk Distribution
          </p>
          ...
          <div className="mt-4 space-y-3">
            {Object.keys(fileTypes).length ? (
              ...
              <p className="text-sm text-app-muted">
                No processed file distribution yet.
              </p>
            )}
          </div>
        </section>
          ...
          <div className="mt-4 space-y-2 text-sm text-app">
            {topTypes.length ? (
              topTypes.map((item) => (
                <p
                  key={item.type}
                  className="flex items-center justify-between border-b border-app pb-2"
                >
                  <span>{item.type}</span>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          {scanTrend.length ? (
            scanTrend.slice(-12).map((point) => (
              <div
                key={point.date}
                className="surface-card rounded-2xl p-3 text-sm text-app-secondary"
              >
...
function ActivityPanel({ events, loading, error, onRetry, planFeatures }) {
  if (!planFeatures?.audit_visibility) {
    return (
      <PlanLockNotice
        ...
        message="Audit trail visibility is available on Pro and Business plans."
      />
    );
  }
  if (loading) {
  ...
  if (!events.length) {
    return (
      <div className="surface-card border-dashed border-cyan-300/30 p-10 text-center text-app-secondary">
        No recent activity yet for this company.
      </div>
    );
  }
  return (
    <div className="surface-card overflow-hidden">
      ...
      {events.map((event) => (
        <div
          key={event.event_id}
          className="grid grid-cols-[170px_150px_1fr_120px] gap-4 border-t border-app px-5 py-4 text-sm text-app-secondary"
        >
          <span>{formatDateTime(event.created_at)}</span>
          <span className="font-semibold text-cyan-700 dark:text-cyan-100">
            {event.event_type}
          </span>
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-4">
        <StatCard
          label="Failed Logins"
          value={data?.failed_login_attempts ?? 0}
          tone="accent"
        />
        <StatCard
          label="Security Alerts"
          value={data?.recent_security_alerts?.length ?? 0}
        />
        <StatCard
      ...
      <section className="surface-card p-5">
        ...
        <div className="mt-4 space-y-3 text-sm text-app-secondary">
          {(data?.recent_security_alerts || []).length ? (
            data.recent_security_alerts.map((alert) => (
              <div key={alert.id} className="rounded-2xl border border-app p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="font-semibold text-app">
                      {alert.event_label ||
                        formatSecurityEventLabel(alert.event_type)}
                    </p>
                ...
                <p className="mt-2 text-xs text-app-muted">
                  Recommended action: review session activity, linked incidents,
                  and active access for the affected actor.
                </p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {(alert.linked_tasks || []).length ? (
                    alert.linked_tasks.map((task) => (
                      <button
                        key={task.id}
                        type="button"
                        onClick={() => navigate("/compliance/tasks")}
        ...
        <div className="mt-4 space-y-3 text-sm text-app-secondary">
          {(data?.incidents || []).length ? (
            data.incidents.map((incident) => (
              <div
                key={incident.id}
                className="rounded-2xl border border-app p-4"
              >
                <p className="font-semibold capitalize text-app">
...
function SettingsPanel({ settingsHook, planFeatures }) {
  const { settings, setSettings, loading, saving, error, saveMessage, save } =
    settingsHook;
  ...
  if (!planFeatures?.company_settings) {
    return (
      <PlanLockNotice
        ...
        message="Company settings are currently available on Pro and Business plans."
      />
    );
  }
  if (loading) {
  ...
  const allowedUploadTypes = Array.isArray(settings.allowed_upload_types)
    ? settings.allowed_upload_types.join(", ")
    : "";
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm text-app-secondary">
          ...
          <input
            ...
            value={settings.default_policy_label || ""}
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                default_policy_label: e.target.value,
              }))
            }
          />
        </label>
        <label className="text-sm text-app-secondary">
          ...
          <input
            className="mt-2 w-full rounded-xl border border-app bg-app px-3 py-2 text-app"
            value={settings.default_report_display_name || ""}
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                default_report_display_name: e.target.value,
              }))
            }
          />
        </label>
        ...
        <label className="text-sm text-app-secondary">
          ...
          <input
            className="mt-2 w-full rounded-xl border border-app bg-app px-3 py-2 text-app"
            value={allowedUploadTypes}
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                allowed_upload_types: e.target.value
                  .split(",")
                  .map((item) => item.trim())
                  .filter(Boolean),
              }))
        ...
        <label className="text-sm text-app-secondary">
          ...
          <input
            ...
            min="1"
            className="mt-2 w-full rounded-xl border border-app bg-app px-3 py-2 text-app"
            value={settings.retention_days_display || ""}
            onChange={(e) =>
              setSettings((prev) => ({
                ...prev,
                retention_days_display: e.target.value,
              }))
            }
          />
        </label>
      ...
      <p className="text-xs text-app-muted">
        Some options are display-ready foundations for upcoming policy and
        retention controls.
      </p>
    </form>
  );
...
function Dashboard({ initialTab = "files", showAdminRail = false }) {
  ...
  const isSecurityAdmin = Boolean(
    user?.permissions?.can_access_security_dashboard,
  );
  ...
  const {
    ...
    reload: reloadCompanyUsers,
  } = useCompanyUsers(isAdmin);
  const adminOverview = useAdminOverview(isAdmin);
  const auditFeed = useAuditEvents(isAdmin && Boolean(plan.audit_visibility));
  const settingsHook = useCompanySettings(
    isAdmin && Boolean(plan.company_settings),
  );
    ...
    <div className="page-shell px-4 py-8 text-app sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="surface-panel overflow-hidden rounded-[2rem] p-8 shadow-2xl backdrop-blur">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(280px,0.7fr)]">
            <div>
              ...
              <h1 className="text-app mt-4 text-4xl font-semibold tracking-tight sm:text-5xl">
                Welcome back, {displayName}
              </h1>
              <p className="text-app-secondary mt-4 max-w-2xl text-sm leading-7 sm:text-base">
                Privacy work should be operational, not aspirational. ALEX helps
                your team detect sensitive data, apply redaction, and ship
                audit-ready outputs with clear controls.
              </p>
            </div>
            <div className="surface-tint rounded-[1.75rem] p-6">
              ...
              <p className="mt-4 text-xl font-semibold text-app">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            label={isAdmin ? "Company Scans" : "Your Scans"}
          ...
          />
          <StatCard
            label={isAdmin ? "Company" : "Profile Email"}
            value={
              userLoading
                ? "..."
                : isAdmin
        ...
        >
          <TabList>
            <Tab value="files">
              {isAdmin ? "Company Scans" : "Redacted Files"}
            </Tab>
            ...
            {isAdmin ? <Tab value="llm">LLM Admin</Tab> : null}
          ...
          <TabPanels>
            ...
            {isAdmin ? (
              <TabPanel value="llm">
                <LLMAdminPanel companyId={userCompanyId} isAdmin={isAdmin} />
              </TabPanel>
            ) : null}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                  ...
                  {userLoading ? (
                    ...
                    <dl className="mt-4 space-y-4 text-sm">
                      <div>
                        ...
                        <dd className="mt-1 font-medium text-app">
                      ...
                      <div>
                        ...
                        <dd className="mt-1 font-medium capitalize text-app">
                          {user?.role || "Unavailable"}
                        </dd>
                      </div>
                      <div>
                        <dt className="text-app-muted">Company</dt>
                        <dd className="mt-1 font-medium text-app">
                      ...
                      <div>
                        ...
                        <dd className="mt-1 font-medium capitalize text-app">
                          {plan.tier || user?.tier || "Unavailable"}
                        </dd>
                      </div>
                    </dl>
                  )}
                ...
                <div className="surface-card p-6">
                  ...
                  <p className="mt-3 text-sm text-app-secondary">
                    Feature visibility is role- and plan-aware to support
                    commercial rollout and predictable user experience.
                  </p>
                  <ul className="mt-4 space-y-2 text-sm text-app">
                    <li>
                      Admin analytics:{" "}
                      {plan.admin_analytics ? "Enabled" : "Locked"}
                    </li>
                    <li>
                      Audit trail visibility:{" "}
                      {plan.audit_visibility ? "Enabled" : "Locked"}
                    </li>
                    <li>
                      Company settings:{" "}
                      {plan.company_settings ? "Enabled" : "Locked"}
                    </li>
              ...
              <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
                <aside className="surface-tint p-6">
                  ...
                  <p className="mt-3 text-sm leading-6 text-app-secondary">
                    Submit a supported file, run detection and redaction, then
                    review your results and reports in the scan activity tab.
                  </p>
                </aside>
