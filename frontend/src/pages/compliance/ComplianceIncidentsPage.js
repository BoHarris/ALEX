...
export default function ComplianceIncidentsPage() {
  ...
  const filteredIncidents = incidents.filter((item) => {
    const matchesSeverity =
      !severityFilter || item.incident.severity === severityFilter;
    const matchesStatus = !statusFilter || item.record.status === statusFilter;
    const matchesOwner =
      !ownerFilter ||
      String(item.incident.assigned_to_employee_id || "") === ownerFilter;
  ...
  async function createRemediationTask() {
    if (!selectedIncident) {
      return;
    }
    setError(null);
    try {
      await workspace.createTaskFromIncident(selectedIncident.incident.id, {});
  ...
  async function createTaskFromSecurityIncident(incident) {
    if (!incident) {
      return;
    }
    setError(null);
    try {
      await workspace.createTaskFromSecurityIncident(incident.id);
      navigate("/compliance/tasks");
    } catch (err) {
      setError(err.message);
    }
  ...
  const timeline = selectedIncident
    ? workspace.timelineCache[selectedIncident.record.id]?.timeline
    : null;
  const columns = [
    {
      key: "record",
      label: "ID",
      render: (row) => (
        <span className="text-app-muted">INC-{row.incident.id}</span>
      ),
    },
    {
      key: "title",
      label: "Title",
      render: (row) => (
        <span className="font-semibold text-app">{row.record.title}</span>
      ),
    },
    {
      key: "severity",
      label: "Severity",
      render: (row) => (
        <span className={statusTone(row.incident.severity)}>
    ...
    },
    {
      ...
      label: "Status",
      render: (row) => (
        <span className={statusTone(row.record.status)}>
          {row.record.status}
        </span>
      ),
    },
    {
      key: "tasks",
      label: "Tasks",
      render: (row) => row.linked_tasks?.length || 0,
    },
    {
      key: "owner",
      label: "Owner",
      render: (row) => row.incident.assigned_to_employee_id || "Unassigned",
    },
    {
      key: "detected",
      label: "Detected At",
      render: (row) => formatDateTime(row.incident.detected_at),
    },
    {
      key: "updated",
      label: "Last Updated",
      render: (row) => formatDateTime(row.record.updated_at),
    },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-app-secondary">
              Serious operational incident management with status transitions
      ...
      {filteredIncidents.length ? (
        ...
        <WorkspaceEmptyState
          ...
          description="Adjust filters or log a new incident."
          action={
            <Button onClick={() => setCreateOpen(true)}>Log Incident</Button>
          }
        />
      ...
      <DetailDrawer
        ...
        title="Log Incident"
        subtitle="Create incidents in a dedicated panel, not inside the dashboard grid."
      >
        <form className="space-y-4" onSubmit={createIncident}>
          ...
          <label className="block text-sm text-app-secondary">
            ...
            <textarea
              ...
              className="mt-2 min-h-40 w-full rounded-xl border border-app bg-app px-3 py-2 text-app"
            />
          </label>
      ...
      <DetailDrawer
        ...
        title={selectedIncident?.record.title || "Incident Detail"}
        subtitle={
          selectedIncident
            ? `${selectedIncident.incident.severity} · ${selectedIncident.record.status}`
            : ""
        }
      >
        {selectedIncident ? (
          <>
            ...
            <section className="surface-card rounded-3xl p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                <Button onClick={() => updateIncident({ status: "contained" })}>
                  Contained
                </Button>
                <Button
                  onClick={() =>
                    updateIncident({
                      status: "resolved",
                      resolution_notes: "Resolution documented.",
                    })
                  }
                >
                  Resolve
                ...
                <Button
                  onClick={() =>
                    updateIncident({
                      status: "closed",
                      resolution_notes: "Closed from workspace.",
                    })
                  }
                >
                  Close Incident
                </Button>
                <Button onClick={createRemediationTask}>
            ...
            <section className="surface-card rounded-3xl p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedIncident.linked_tasks?.length ? (
                  ...
                  <p className="text-sm text-app-muted">
                    No remediation tasks linked yet.
                  </p>
                )}
              </div>
            </section>
            <section className="surface-card rounded-3xl p-5">
              ...
              <p className="mt-4 text-sm text-app-secondary">
                {selectedIncident.incident.resolution_notes ||
            ...
            <section className="surface-card rounded-3xl p-5">
              ...
              <p className="mt-4 text-sm text-app-secondary">
                {selectedIncident.incident.lessons_learned ||
                  "Lessons learned not documented yet."}
              </p>
            </section>
            <section className="surface-card rounded-3xl p-5">
              ...
              <div className="mt-4 space-y-3">
                {timeline?.activities?.length ? (
                  ...
                  <p className="text-sm text-app-muted">
                    No incident timeline entries yet.
                  </p>
                )}
              </div>
            </section>
